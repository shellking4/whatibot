This is simple REST API wrapper around the whatsapp web js library, which is a whatsapp automation toolkit.

The project includes a staic/index.html file where we use socket.io to get the qr code in realtime.

Here is a way you can get everything working :

Normally when the server starts the whatsapp web authentication should happen automatically. Bu sometimes it doesn't. So as a rule of thumb you can do these steps:

1- Open the endpoint / in your browser. You should a starter invalid whatsapp Qr code
...
It is possible that the whatsapp web auth credentials which are stored in .wwebjs_auth get corruped. In which case you need to do the authentication process (ie. steps 1, 2, 3)
